...
namespace RSR
{
    ...
    SerialPortInterface::SerialPortInterface(const SerialPort::Ptr& ptr)
        :m_serial_ptr(ptr)
    {

        // frame_date_lenth = FRAME_DATA_LENGTH;
    }
    ...
    void SerialPortInterface::packFrame(const std::vector<uint8_t> &data, std::vector<uint8_t> &frame)
    {
    ...
    bool SerialPortInterface::parseFrame(const std::vector<uint8_t> &frame, std::vector<uint8_t> &data)
    {
    ...
    std::vector<uint8_t> SerialPortInterface::receiveData()
    {
        ...
        try
        {
            ...
            {
                DataBuffer byte;
                m_serial_ptr->read(byte, 1); // 每次读取1字节，超时10ms
    ...
    uint16_t SerialPortInterface::calculateCRC16(const uint8_t *data, uint16_t length)
    {
        uint16_t crc = 0xFFFF;
    ...
    void SerialPortInterface::PackAndSend(const std::vector<uint8_t>& data){
        std::vector<uint8_t> frame;
        packFrame(data, frame);
        m_serial_ptr->send(frame);
    }
