...
void MotorControl::ControlMotors(serialport::SerialPortWrapper &port, float pos[6], float vel[6], float kp[6], float kd[6], float tau[6])
{
    ...
    {
        if(pos[i]>m_desir_motor_pos_protect_max[i]){
            std::cout << "joint" << i << " desir pos out range:" << " " << pos[i]<<std::endl;
            pos[i] = m_desir_motor_pos_protect_max[i];
        }
        if (pos[i] < m_desir_motor_pos_protect_min[i])
        {
            std::cout << "joint" << i << " desir pos out range:" << " " << pos[i] << std::endl;
            pos[i] = m_desir_motor_pos_protect_min[i];
        }
...
void MotorControl::updatMotorState(std::vector<float> motorState)
{
    ...
    // std::cout << "motorState:" << motorState[8] << std::endl;

}

void MotorControl::real_time_motor_protection(float vel_[MOTOR_NUM], float tau_[MOTOR_NUM], serialport::SerialPortWrapper &port)
{
